...
def main():
    ...
    group.add_argument('channel_url', nargs='?', help='YouTubeチャンネルのURL')
    group.add_argument('--channel-id', help='YouTubeチャンネルID（直接指定する場合）')
    ...
    if args.channel_id:
        ...
        if not channel_info:
            print(f"エラー: チャンネルID '{channel_id}' の情報が取得できませんでした。")
            return 1
        
        livestreams = extractor.get_livestreams(channel_id, args.max_results)
