...
def get_youtube_service():
    """YouTube Data API v3のサービスを取得する（OAuth2認証）"""
    print("YouTube APIサービスを初期化します（OAuth2認証）")
    
    creds = None
    
    # トークンファイルが存在する場合は読み込む
    if os.path.exists(TOKEN_PICKLE_FILE):
        print(f"既存のトークンファイル {TOKEN_PICKLE_FILE} を読み込みます")
...
def create_client_secrets_file():
    """環境変数からクライアントシークレットファイルを作成する"""
    print("クライアントシークレットファイルを作成します")
    
    if not YOUTUBE_CLIENT_ID or not YOUTUBE_CLIENT_SECRET:
        print("YOUTUBE_CLIENT_IDまたはYOUTUBE_CLIENT_SECRETが設定されていません")
        return False
    
    client_secrets = {
        "installed": {
            "client_id": YOUTUBE_CLIENT_ID,
            "project_id": "youtube-transcript-extractor",
    ...
    print(f"クライアントシークレットファイルを {CREDENTIALS_FILE} に保存しました")
    return True
...
考えられる原因:
1. YouTube APIの認証に失敗した（OAuth2認証が必要）
2. 対象の動画に字幕が存在しない
3. APIのクォータ制限に達した
...
def main():
    ...
    if not YOUTUBE_CLIENT_ID or not YOUTUBE_CLIENT_SECRET:
        ...
        print("YOUTUBE_CLIENT_IDとYOUTUBE_CLIENT_SECRETが設定されています")
