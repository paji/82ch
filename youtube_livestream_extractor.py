...
def check_transcript_exists(owner, repo, video_id):
    """指定されたビデオIDの文字起こしファイルが既に存在するか確認する"""
    url = f"https://api.github.com/repos/{owner}/{repo}/contents/{video_id}.md"
    response = requests.get(url, headers=GITHUB_HEADERS)
    return response.status_code == 200

def save_transcript_to_repo(owner, repo, video_id, transcript, video_title):
    """文字起こしをGitHubリポジトリに保存する"""
    # ファイル名とコンテンツを準備
...
"""
    ...
    data = {
        "message": f"Add transcript for {video_id}",
    ...
    if response.status_code in [201, 200]:
        print(f"Successfully saved transcript for {video_id}")
        return True
    else:
        print(f"Failed to save transcript: {response.status_code}, {response.text}")
        return False
...
def main():
    ...
    for stream in livestreams:
        ...
        # 既に文字起こしが存在するかチェック
        if check_transcript_exists(target_owner, target_repo, video_id):
        ...
        # 文字起こしを保存
        success = save_transcript_to_repo(target_owner, target_repo, video_id, transcript, video_title)
